...
|noRedirect       |`Boolean` | If set to `true` the `authorize` function will instead return a string containing the redirect URL instead of actually redirecting. This can be useful during testing or when additional actions must be taken before redirection.
