...
|issMatch|`string` or `RegExp` or `function`|Since version `2.3.11`, in case of EHR launch the `authorize` function can also be called with an array of options. This makes it possible to pre-configure an app to be launch-able from multiple EHR systems. The right configuration will be picked based on the passed `iss` url parameter. [Read More](http://docs.smarthealthit.org/client-js/open_servers#2-using-multiple-launch-configurations)
